...
import { DEFAULT_IMAGE_MODEL, getImageModelMapBySet } from "@/api/consts/common.ts";
...
function getRegionDisplayName(regionInfo: RegionInfo): string {
  switch (regionInfo.countryCode) {
    case "hk":
      return "香港";
    case "jp":
      return "日本";
    ...
    case "intl-default":
      return "国际默认";
    default:
      return regionInfo.countryCode.toUpperCase();
  }
}
...
export function getModel(model: string, regionInfo: RegionInfo): ModelResult {
  const modelMap = getImageModelMapBySet(regionInfo.profile.modelSet);
  const defaultModel = regionInfo.profile.defaultImageModel || DEFAULT_MODEL;
...
function logResolutionInfo(userModel: string, resolution: ResolutionResult, regionInfo: RegionInfo) {
  ...
  if (userModel === 'nanobanana') {
    if (regionInfo.isUS) {
      ...
      const regionName = getRegionDisplayName(regionInfo);
