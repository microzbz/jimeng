...
import { DEFAULT_VIDEO_MODEL, DRAFT_VERSION, DRAFT_VERSION_OMNI, OMNI_BENEFIT_TYPE, OMNI_BENEFIT_TYPE_FAST, getVideoModelMapBySet } from "@/api/consts/common.ts";
import { DA_VERSION } from "@/api/consts/dreamina.ts";
import { getGenerateReferer } from "@/api/consts/country-profiles.ts";
...
export function getModel(model: string, regionInfo: RegionInfo) {
  const modelMap = getVideoModelMapBySet(regionInfo.profile.modelSet);
  const defaultModel = regionInfo.profile.defaultVideoModel || DEFAULT_MODEL;
  return modelMap[model] || modelMap[defaultModel] || modelMap[DEFAULT_MODEL];
}

export async function ensureVideoComplianceConfirmed(refreshToken: string, regionInfo: RegionInfo): Promise<void> {
  if (!regionInfo.isInternational) {
    return;
  }

  try {
    const result = await request("post", "/mweb/v1/get_settings", refreshToken);
    const confirmed = result?.user_custom_settings?.aigc_compliance_confirmed;
    if (confirmed) {
      logger.info("国际站视频合规确认已完成");
      return;
    }
  } catch (error: any) {
    logger.warn(`读取视频合规确认状态失败，继续尝试自动确认: ${error.message}`);
  }

  logger.info("检测到国际站视频合规确认未完成，尝试自动确认...");
...
function getOmniBenefitType(model: string, regionInfo: RegionInfo): string {
  if (regionInfo.isInternational) {
    return getVideoBenefitType(model);
  }

  const is40 = model.includes("40") && !model.includes("40_pro");
  return is40 ? OMNI_BENEFIT_TYPE_FAST : OMNI_BENEFIT_TYPE;
}
...
export function getVideoBenefitType(model: string): string {
  ...
  if (model.includes("unified_edit_model")) {
    return "dreamina_video_unified_edit";
  }
...
export async function generateVideo(
  ...
  await ensureVideoComplianceConfirmed(refreshToken, regionInfo);
  ...
  const supportsOmniReference = is40Pro || is40 || model.includes("unified_edit_model");

  // omni_reference 仅支持 seedance 2.0 / 2.0-fast / Video Lab
  if (isOmniMode && !supportsOmniReference) {
    throw new APIException(EX.API_REQUEST_FAILED,
      `omni_reference 模式仅支持 jimeng-video-seedance-2.0、jimeng-video-seedance-2.0-fast 和 jimeng-video-video-lab 模型`);
  }
  ...
  if (isOmniMode) {
    ...
    // 根据模型选择 benefit_type
    const omniBenefitType = getOmniBenefitType(model, regionInfo);
  ...
  // 发送请求
  const videoReferer = getGenerateReferer(regionInfo.profile, "video");
