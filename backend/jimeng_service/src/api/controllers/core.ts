...
import { CountryProfile, getCountryProfile, getHomeReferer } from "@/api/consts/country-profiles.ts";
...
export interface ParsedTokenContext extends TokenWithProxy {
  rawToken: string;
  sessionToken: string;
  countryCode: string;
  profile: CountryProfile;
...
function parseTokenPrefix(token: string): { countryCode: string; sessionToken: string } {
  const normalized = token.trim();
  const match = normalized.match(/^([a-z]{2}|intl)-(.+)$/i);

  if (!match) {
    return { countryCode: "cn", sessionToken: normalized };
  }

  const [, prefix, rest] = match;
  const countryCode = prefix.toLowerCase();

  if (countryCode === "cn") {
    return { countryCode, sessionToken: rest };
  }

  return { countryCode, sessionToken: rest };
}

export function parseTokenContext(rawToken: string): ParsedTokenContext {
  const { token, proxyUrl } = parseProxyFromToken(rawToken);
  const { countryCode, sessionToken } = parseTokenPrefix(token);
...
export function parseRegionFromToken(refreshToken: string): RegionInfo {
  const { countryCode, profile } = parseTokenContext(refreshToken);
  const isUS = profile.code === "us";
  const isHK = profile.code === "hk";
  const isJP = profile.code === "jp";
  const isSG = profile.code === "sg" || profile.code === "intl-default";
  const isInternational = profile.siteFamily === "international";

  return {
    countryCode,
    profileKey: profile.code,
    profile,
    isUS,
    isHK,
    isJP,
...
export function getRefererByRegion(refreshToken: string, cnPath: string): string {
  const { profile } = parseRegionFromToken(refreshToken);
  if (profile.siteFamily === "international") {
    return profile.webOrigin;
  }
  return `${profile.webOrigin}${cnPath}`;
}
...
export function getAssistantId(regionInfo: RegionInfo): number {
  return regionInfo.profile.assistantId;
}
...
export function generateCookie(refreshToken: string) {
  const { sessionToken: token } = parseTokenContext(refreshToken);
...
export async function receiveCredit(refreshToken: string) {
  ...
  const { profile } = parseRegionFromToken(refreshToken);
  const referer = getHomeReferer(profile);
...
export async function request(
  ...
  const tokenContext = parseTokenContext(refreshToken);
  const { token: tokenWithRegion, proxyUrl, sessionToken, profile } = tokenContext;
  ...
  const baseUrl = uri.startsWith("/commerce/") ? profile.commerceBaseUrl : profile.requestBaseUrl;
  const aid = profile.assistantId;
  const region = profile.regionParam;
  ...
  const requestParams = options.noDefaultParams ? (options.params || {}) : {
    ...
    device_platform: "web",
    region: region,
    ...(profile.includeWebId ? { webId: WEB_ID } : {}),
...
export async function getTokenLiveStatus(refreshToken: string) {
  try {
    ...
    // 国际站（含 tw/hk/jp/sg/us）不再走旧的 passport/account/info/v2，
    // 该接口在新国际链路上会返回 404。改用已验证可用的积分接口做存活检测。
    if (profile.siteFamily === "international") {
